  ...
  function attachL(screen,data){
    ...
    if(screen==='send'){
      ...
      const loadBankList = (banks) => {
        const container = c.querySelector('#bankListContainer');
        const hiddenInput = c.querySelector('#bankName');
        if(!container) return;
        ...
        container.querySelectorAll('.bank-item').forEach(item=>{
          item.addEventListener('click', ()=>{
            const bs = c.querySelector('#bankSearch');
            const sd = c.querySelector('#selectedBankDisplay');
      ...
      resolveTimer = setTimeout(() => {
        fetch('/api/wallet/resolve', {
          ...
          if(data.success){
            ...
            if(ngnSupported){
              resolvedEl.textContent = '❌ ' + (data.message || 'Account not found — check details');
    ...
    c.querySelectorAll('.send-contact-btn').forEach(b=>b.addEventListener('click',()=>{sCont={id:b.dataset.id,name:b.dataset.name};c.querySelector('#bankName')?.addEventListener('change', () => {
      const accInput = c.querySelector('#bankAccount');
      if(accInput && accInput.value.length >= 10) accInput.dispatchEvent(new Event('input'));
    });
    c.querySelectorAll('.send-contact-btn').forEach(x=>x.style.borderColor='rgba(255,255,255,0.08)');b.style.borderColor='#00B0A0';const disp=c.querySelector('#selContactDisplay');disp.style.display='block';disp.textContent='Selected: '+sCont.name;}));
