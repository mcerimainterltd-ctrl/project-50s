...
// ── Device Contacts Permission & Sync ────────────────────────────────────
async function requestContactsPermission() {
  try {
    if (!window.Capacitor?.isNativePlatform?.()) return;
    const Contacts = window.Capacitor?.Plugins?.Contacts;
    if (!Contacts) { console.warn('Contacts plugin not available'); return; }
    ...
    if (permission === 'granted') {
      ...
      showNotification('Contacts access denied. Enable it in Settings → XamePage → Permissions.');
    }
  } catch (e) {
    console.warn('Contacts permission error:', e.message);
  }
}
...
async function syncDeviceContacts() {
  try {
    ...
    console.log('Device contacts fetched:', result.contacts.length);
  } catch (e) {
    console.warn('Sync device contacts error:', e.message);
  }
}
