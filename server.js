...
app.post('/api/register',
    ...
    async (req, res) => {
        ...
        const { firstName, lastName, dob, password, referralCode: submittedReferralCode } = req.body;
        try {
            const xameId         = await generateUniqueXameId();
            const hashedPassword = await bcrypt.hash(password, 10);
            const newUserReferralCode = xameId.replace('@', '').toUpperCase() + Math.random().toString(36).substring(2, 6).toUpperCase();
            ...
            // ── Ensure every new user has a RewardAccount ───────────────────
            // This is created even when the user did not join through a referral.
            let userRewardAccount = await RewardAccount.findOne({ userId: xameId });
            ...
            if (!userRewardAccount) {
                ...
                userRewardAccount = await RewardAccount.create({
                    userId: xameId,
                    referralCode: rewardCode
                });

                console.log(`✅ RewardAccount created: ${xameId}`);
            }

            // ── Process referral server-side during registration ─────────────
            if (submittedReferralCode) {
                try {
                    let referrerAccount = await RewardAccount.findOne({
                        referralCode: submittedReferralCode
                    });

                    // Fall back to the User model's referral code.
                    if (!referrerAccount) {
                        const referrerUser = await User.findOne({
                            referralCode: submittedReferralCode
                        });
                        if (referrerUser) {
                            referrerAccount = await RewardAccount.findOne({
                                userId: referrerUser.xameId
                            });
                        }
                    }

                    if (referrerAccount && referrerAccount.userId !== xameId) {
                        // Ensure the new user has a reward account.
                        let newRewardAccount = await RewardAccount.findOne({
                            userId: xameId
                        });
                        ...
                        if (!newRewardAccount) {
                            ...
                            newRewardAccount = await RewardAccount.create({
                                userId: xameId,
                                referralCode: rewardCode,
                                referredBy: referrerAccount.userId
                            });
                        } else if (!newRewardAccount.referredBy) {
                            newRewardAccount.referredBy = referrerAccount.userId;
                            await newRewardAccount.save();
                        }

                        // Award the referrer exactly once.
                        ...
                        if (!alreadyRewarded) {
                            ...
                            referrerAccount.tier =
                                getTier(referrerAccount.activeReferrals);
                            ...
                            if (referrerAccount.monthlyReferrals >= 5) {
                                ...
                                if (!benchmarkAwarded) {
                                    ...
                                    console.log(
                                        `🏆 Monthly referral benchmark awarded: ${referrerAccount.userId}`
                                    );
                                }
                            }

                            console.log(
                                `✅ Referral rewarded: ${referrerAccount.userId} → ${xameId}`
                            );
                        }

                        // Automatically connect the two users.
                        try {
                            const referrerUser = await User.findOne({
                                xameId: referrerAccount.userId
                            });

                            if (referrerUser) {
                                const now = new Date();

                                if (!(user.contacts || []).some(
                                    c => String(c.contactId) === String(referrerUser._id)
                                )) {
                                    user.contacts.push({
                                        contactId: referrerUser._id,
                                        addedAt: now
                                    });
                                }

                                if (!(referrerUser.contacts || []).some(
                                    c => String(c.contactId) === String(user._id)
                                )) {
                                    referrerUser.contacts.push({
                                        contactId: user._id,
                                        addedAt: now
                                    });
                                }
                                ...
                                console.log(
                                    `✅ Referral auto-connected: ${referrerAccount.userId} ↔ ${xameId}`
                                );
                            }
                        } catch (connectErr) {
                            console.warn(
                                `⚠️ Referral auto-connect failed:`,
                                connectErr.message
                            );
                        }
                    }
                } catch (referralErr) {
                    // Registration must still succeed even if referral processing fails.
                    console.warn(
                        `⚠️ Referral processing failed for ${xameId}:`,
                        referralErr.message
                    );
                }
            }
...
app.get('/join/:code', async (req, res) => {
    try {
        ...
        const apkUrl = `https://app.xamepage.com/api/app/download?ref=${encodeURIComponent(code)}`;
